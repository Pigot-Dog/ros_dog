...
class XYZ_CONTROL(object):
    def __init__(self, h, v_E1, v_E2, steps_num):
        ...
        self.x = np.zeros(self.step_num / 2)
        self.y = np.zeros(self.step_num / 2)
        self.z = np.zeros(self.step_num / 2)
        self.theta = np.linspace(0, 2 * math.pi, self.step_num / 2)
         
    def x_dict(self, control_h):
        self.h = control_h 
        
        for i in range(self.step_num / 2):
            self.x[i] = self.h * (1 - math.cos(self.theta[i])) / 2
        
        self.x = -self.x
        self.x_gait = np.hstack((self.x[10:20], self.x[0:20]**0*self.x[19], self.x[0:10]))
        return self.x_gait
    
    def y_dict(self, control_v_E2):
        self.E_lateral = control_v_E2
        
        for i in range(self.step_num / 2):
            self.y[i] = self.E_lateral * (self.theta[i] - math.sin(self.theta[i])) / (2 * math.pi)
        
        self.y = self.y
        self.y_gait = np.hstack((self.y[10 : 20], np.linspace(self.y[19], self.y[0], self.step_num / 2),
                                 self.y[0 : 10]))
        return self.y_gait
    
    def z_dict(self, control_v_E1):
        self.E_long = control_v_E1
        
        for i in range(self.step_num / 2):
            self.z[i] = self.E_long * (self.theta[i] - math.sin(self.theta[i])) / (2 * math.pi)
        
        self.z = self.z 
        self.z_gait = np.hstack((self.z[10 : 20], np.linspace(self.z[19], self.z[0], self.step_num / 2),
                                 self.z[0 : 10]))
        return self.z_gait
...
def turn_gait(direction):
    ...
    old_foot_y = np.array([-0.35,  0.35, 0.35,  -0.35]) # The width direction [-0.35,  0.35, -0.35,  0.35]
    old_foot_z = np.array([ 0.40,  0.40, -0.40, -0.40]) # The length direction [ 0.40,  0.40, -0.40, -0.40]
